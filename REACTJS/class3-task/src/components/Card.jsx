const Card = (props) => {
  ...
  return (
    <div className="card sm:flex-col sm:px-1 sm:mx-0 rounded-3xl flex flex-row justify-center h-fit items-center ">
      ...
      {/* Top Bar Animate */}
        <div className="group border-b-1 backdrop-blur-xs flex flex-row justify-between items-center text-xl font-bold text-white bg-black/50 w-full text-center py-2 px-4 mt-2 rounded-3xl  transform md:translate-y-6 md:opacity-0
          transition-all duration-500 ease-out group-hover:translate-0 group-hover:opacity-100">  
          <img className="h-[30px] w-[30px] rounded-full border-gray-300 border-1 object-cover" src={props.profile} alt="img" />
          
            <h2 className="text-sm font-semibold">{props.fullname}</h2>
            {/* Social Icons (Animate) */}
          <div className="text-amber-100 font-light flex flex-row justify-center items-center gap-1  transform sm:translate-y-6 sm:opacity-0
          transition-all duration-500 ease-out group-hover:translate-0 group-hover:opacity-100">
